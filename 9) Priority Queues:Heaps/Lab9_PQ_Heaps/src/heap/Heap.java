...
public class Heap<E> extends CompleteBinaryTree<E> {
	...
	private void downHeap(CBTPosition<E> r) { 
		if (this.hasLeft(r)) { 
			...
			/*While a parent is bigger than one of it's children, we have broken the heap property*/
			if(cmp.compare(r.getElement(), minChild.getElement()) > 0) {
				swapPositionsInList(r,minChild); // Sifted down in the tree to try and restore the heap property
				downHeap(r);
			} else {
				return; //met the heap property, so we can just return ] 
			}
