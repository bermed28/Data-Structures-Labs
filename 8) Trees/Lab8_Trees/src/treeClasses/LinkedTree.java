...
public class LinkedTree<E> extends AbstractTree<E> implements Cloneable {
	...
	public Position<E> addChild(Position<E> p, E element) 
			throws IllegalArgumentException { 
		Node<E> np = validate(p);  
		Node<E> newNode = new Node<>(element, np, null, this); 
		if (np.getChildren() == null)
			np.setChildren(new ArrayList<Node<E>>());
		np.getChildren().add(newNode); 
		size++; 
		return newNode; 
	}
	...
	public E remove(Position<E> p) throws IllegalArgumentException { 
		...
		if (parent == null)    // then ntd is the root
			...
			if(ntd.getChildren() != null) { //Fix for error specified in e-mail that appeared when trying to remove a leaf
				
				for (Node<E> childNTD : ntd.children) { 
					parent.getChildren().add(childNTD);   
					// Exercise: why can't we use addChild method here????
					/*Because if we were to use it, it would ruin the BST Structure*/
		...
		/**
		 * What was missing is that we "detached" all of ntd's children 
		 * and "connected" them to parent.
		 * 
		 * However, we still need to "detach" ntd from the parent, 
		 * so we iterate through all of parents children to see if we find ntd
		 * When we do, we just remove it.
		 * */
		if(parent != null) {
	...
	public LinkedTree<E> clone() throws CloneNotSupportedException { 
		...
		/**
		 * EXCERSICE 7 SOLUTION AND EXPLANATION:
		 * This method throws a NullPointerException when it tries to clone an empty Tree.
		 * 
		 * It throws the exception because it tries to find a root to start the cloning, 
		 * where in an empty tree there IS NO ROOT.
		 * 
		 * To avoid this, we just check if the root is null, 
		 * if it's null it means the tree is empty so we just return an empty tree
		 * 
		 * Otherwise, we clone the rest of the tree like normal
		 */ 
		if(root() != null) {
